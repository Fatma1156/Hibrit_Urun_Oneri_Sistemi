...
def select_dbscan_params(X, n_features: int) -> tuple[float, int]:
    """DBSCAN eps ve min_samples değerlerini k-distance yaklaşımıyla seçer."""
    min_samples = max(5, 2 * n_features)
    n_neighbors = min(min_samples, len(X))

    if n_neighbors < 2:
        eps = 0.5
        distances_sorted = np.array([eps])
    else:
        ...
        if len(distances_sorted) > 2:
            second_derivative = np.diff(distances_sorted, n=2)
            elbow_idx = int(np.argmax(second_derivative) + 2)
            eps = float(distances_sorted[elbow_idx])
        else:
            eps = float(distances_sorted[-1])

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(distances_sorted, color="steelblue")
    ax.axhline(y=eps, color="red", linestyle="--", label=f"eps={eps:.4f}")
    ax.set_title("DBSCAN k-distance Grafiği")
    ax.set_xlabel("Sıralı gözlem")
    ax.set_ylabel(f"{n_neighbors}. komşu uzaklığı")
    ax.legend()
    plt.tight_layout()
    plt.savefig("outputs/figures/dbscan_k_distance.png")
    plt.close()
    ...
    }]).to_csv("outputs/reports/dbscan_params.csv", index=False)

    print("  DBSCAN parametre seçimi:")
    print(f"  min_samples = {min_samples}")
    print(f"  eps = {eps:.4f}")
    return eps, min_samples


def run_dbscan(X, n_features: int):
    eps, min_samples = select_dbscan_params(X, n_features)
